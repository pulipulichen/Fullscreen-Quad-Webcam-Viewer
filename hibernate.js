...
const server = http.createServer((req, res) => {
  if (req.method === 'GET') {
    console.log('Received GET request. Executing sudo update-grub...');
    exec('sudo update-grub', (error, stdout, stderr) => {
      if (error) {
...
server.listen(port, () => {
  ...
  console.log(`Access http://localhost:${port} to trigger 'sudo update-grub'`);
  console.warn('WARNING: Executing "sudo update-grub" requires sudo privileges and may prompt for a password.');
